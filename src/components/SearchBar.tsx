...
const SearchBar = ({ onSearch, loading }: SearchBarProps) => {
  ...
  const cityButtonClasses = actualTheme === 'dark' 
  ...
  return (
    <div className={`${cardClasses} backdrop-blur-xl rounded-2xl p-4 border shadow-xl transition-all duration-500`}>
      <form onSubmit={handleSubmit} className="flex gap-3 mb-4">
        <div className="relative flex-1">
          <MapPin className={`absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 ${actualTheme === 'dark' ? 'text-gray-400' : 'text-gray-500'} transition-colors duration-500`} />
          <Input
            type="text"
